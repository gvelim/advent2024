...
impl DiskMap {
    ...
    fn insert_file(&mut self, idx: usize, file: Entry) -> &mut Self {
        ...
        if self.0.get(idx).unwrap().0 < file.0 {
            return self;
        }
        let space = self.0.remove(idx);
        self.0
            .splice(idx..idx, [(0, -1), file, (space.0.abs_diff(file.0), -1)]);
        self
    }
    ...
    fn move_file(&mut self, src: usize, dst: usize) -> &mut Self {
        ...
        if self.0.get(src).is_none() || self.0.get(dst).is_none() {
            return self;
        }

        let file = self.0[src];
        if self.0[dst].0 >= self.0[src].0 {
            self.insert_file(dst, file).remove_file(src + 2);
        } else {
    ...
    pub fn expand_diskmap(&self) -> impl Iterator<Item = Entry> {
        self.0
            .iter()
            .flat_map(move |&(count, id)| (0..count).map(move |_| (count, id)))
    }
    ...
    pub fn defragment(&mut self) -> &DiskMap {
        ...
        for file in files.iter().rev() {
            let Some(f_pos) = self.0.iter().rev().position(|e| e == file) else {
                continue;
            };
            let Some(s_pos) = self.spaces().position(|space| space.0 >= file.0) else {
                continue;
            };
            if s_pos * 2 + 1 >= len - f_pos {
                continue;
            }
...
impl Debug for DiskMap {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        for c in self.expand_diskmap().map(|(_, i)| {
            if i == -1 {
                '.'
            } else {
...
mod test {
    ...
    fn test_diskmap_move_file() {
        ...
        assert_eq!(
            dm.move_file(4, 1).0,
            vec![(2, 0), (0, -1), (1, 2), (2, -1), (3, 1), (6, -1), (3, 3)]
        );
        println!("{dm:?}");
        assert_eq!(
            dm.move_file(6, 3).0,
