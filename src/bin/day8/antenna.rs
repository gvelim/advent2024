...
impl Antenna {
    pub fn antinode_pair(&self, rhs: Antenna, harmonics: usize) -> [Option<Location>; 2] {
        ...
        match (self.0.0 >= rhs.0.0, self.0.1 >= rhs.0.1) {
            (true, true) => [
                rhs.0.move_relative((-dx, -dy)),
                self.0.move_relative((dx, dy)),
            ],
            (true, false) => [
                rhs.0.move_relative((-dx, dy)),
                self.0.move_relative((dx, -dy)),
...
mod test {
    ...
    fn test_antinode() {
        ...
        assert_eq!(
            a[0].antinode_pair(a[1], 1),
            [Some(Location(6, 7)), Some(Location(3, 1))]
        );
        assert_eq!(
            a[0].antinode_pair(a[2], 1),
            [Some(Location(12, 5)), Some(Location(0, 2))]
        );
        assert_eq!(
            a[1].antinode_pair(a[2], 1),
            [Some(Location(11, 3)), Some(Location(2, 6))]
        );
    }
}
