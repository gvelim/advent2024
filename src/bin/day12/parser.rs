...
pub(super) fn extract_ranges(line: &str) -> impl Iterator<Item = PlotSegment> {
    let mut idx = 0;
    line.as_bytes().chunk_by(|a, b| a == b).map(move |chunk| {
        let start = idx;
...
fn push_segments(
    plots: &mut HashMap<usize, Plot>,
    g_line: impl Iterator<Item = (PlotSegment, usize, bool)>,
    line: usize,
) {
    g_line.for_each(|(seg, id, _)| {
...
fn process_line(
    ...
    let (mut plots, mut g_line) =
        extract_ranges(input).fold((plots, g_line), |(plots, g_line), segment| {
            // process segment against last Garden Scan Line
            let (mut plots, mut g_line, seg_id, depr_ids) =
...
pub fn parse_plots(input: &str) -> HashMap<usize, Plot> {
    ...
    let (mut plots, mut g_line) = input.lines().fold(
        (HashMap::<usize, Plot>::new(), LastGardenScanLine::default()),
        |(plots, g_line), input| {
            process_line(
                input,
                plots,
...
mod tests {
    use crate::parser;
    use std::collections::HashMap; // Import items from the parent module (parser)
    ...
    fn test_garden_parser() {
        ...
        for (file, results) in test_runs {
            let garden = parser::parse_plots(&std::fs::read_to_string(file).unwrap());
            println!("\nTest Run ===========================");
            for (id, plot) in garden {
                println!("ID:{id}\n{plot:?}");
                let (a, b) = (plot.area(), plot.perimeter_count());
